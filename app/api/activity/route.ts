...
export async function GET() {
  ...
  try {
    const [transfers, transactions, counters] = await Promise.all([
      blockscout(`/tokens/${CONTRACT}/transfers`, apiKey),
    ...
    const events = (transfers?.items ?? [])
      ...
      .map((item: any) => {
        ...
        const tx = txHash
          ? txMap.get(String(txHash).toLowerCase())
          : undefined;
    ...
    return NextResponse.json(
      {
        ...
        holders: Number(
          counters?.token_holders_count ?? 0
        ),

        message: events.length
          ? ""
          : "No NFT activity found yet on Robinhood Chain.",
      },
      {
        headers: {
          "Cache-Control":
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to load Blockscout activity.";
